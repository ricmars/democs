        ...
        AtmosphereRequest: function (options) {
            ...
            function _getWebSocket(location) {
                try {
                    ...
                    atmosphere.util.log("warn", [e.message]);
                }
            }
            ...
            function _executeWebSocket(webSocketOpened) {
                ...
                _websocket.onclose = function (message) {
                    ...
                    if (_canLog('warn')) {
                        atmosphere.util.warn("Websocket closed, reason: " + reason + ' - wasClean: ' + message.wasClean + ' - Code: ' + message.code);
                    }
            ...
            function _f(response, f) {
                switch (response.state) {
                    case "messageReceived":
                        ...
                        if (_request && typeof (_request.reconnectingOnErrorProcess) !== 'undefined' && _request.reconnectingOnErrorProcess === true) {        
                            /* BUG-272417: As error messages also sending as regular messages,in case of error we should skipping reset _requestCount when reconnectingOnErrorProcess is true */        
                        } else {
                            _requestCount = 0;
                        }
...
pega.u.pushservice_settings = pega.u.pushservice_settings || ({
    transport: (deferredVars["pega.u.d"].pushservice_transport) ? deferredVars["pega.u.d"].pushservice_transport : "",
    fallback: (deferredVars["pega.u.d"].pushservice_fallback) ? deferredVars["pega.u.d"].pushservice_fallback : "",
    requestorContextPath: (deferredVars["pega.u.d"].pushservice_requestorContextPath) ? deferredVars["pega.u.d"].pushservice_requestorContextPath : "",
    pushServletPath: (deferredVars["pega.u.d"].pushservice_pushServletPath) ? deferredVars["pega.u.d"].pushservice_pushServletPath : "",
    portalName: (deferredVars["pega.u.d"].pushservice_portalName) ? deferredVars["pega.u.d"].pushservice_portalName : "",
    requestorID: (deferredVars["pega.u.d"].pushservice_requestorID) ? deferredVars["pega.u.d"].pushservice_requestorID : ""
});
pega.u.MessagingManager = pega.u.MessagingManager || (function(p, settings) {
    var testObject = {};
    "use strict";
    /* Register all subscriptions on load and on every ajax  */
    p.u.d.attachOnload(function(elem) {
        if (!elem) {
            elem = document;
        } else if (elem.target) {
            elem = elem.target;
        }
        $(elem).find("[data-subscription-id]").addBack("[data-subscription-id]").each(function() {
            p.u.MessagingManager.registerSubscriber($(this).attr('data-subscription-id'), this);
        });
    }, true);

    if (pega.atmosphere && pega.atmosphere.util && pega.atmosphere.util.debug && pega.atmosphere.util.info) {
        pega.atmosphere.util.debug = function() {
            //disabled debug logs
        }
    ...
    if (p.d.s.desktopWindow && p.d.s.desktopWindow !== window && !p.d.s.desktopWindow.pega.u){
        p.d.s.desktopWindow = p.d.s.getDesktopWindow(); //assigning latest desktopWindow context
    }

    if (p.d.s.desktopWindow && p.d.s.desktopWindow !== window) {
        setTimeout(function() {
            p.d.s.desktopWindow.pega.u.MessagingManager.tryReconnect();
        }, 0);
        return p.d.s.desktopWindow.pega.u.MessagingManager;
    }
    ...
    var log = function(msg) {
        ...
        /* Maintains active subscriptions in key(subscribtion id) value(subscribtion info) pair*/
        ActiveSubscriptionMap = (function() {
            var activeSubscriptions = new p.tools.Hashtable();

            return {
                add: function(subscribtionID, subscriptionInfo) {
                    activeSubscriptions.put(subscribtionID, subscriptionInfo);
                },

                get: function(subscribtionID) {
                    return activeSubscriptions.get(subscribtionID);
                },

                remove: function(subscribtionID, removeAll) {
                    var retVal = null;
                    if (removeAll) {
                        activeSubscriptions.clear();
                        retVal = true;
                    } else {
                        retVal = activeSubscriptions.remove(subscribtionID);
                    }
                    return retVal;
                },
                getAllActiveSubscriptionIDs: function() {
                  var subscriptionIDS = activeSubscriptions.values().map(function(subscriptionMap) {
                    return subscriptionMap.subscribtionID
                  });
                  
                  var activeSubscriptionIDs = subscriptionIDS.filter(function(id) {
                    return $("[data-subscription-id='" + id + "']").length > 0
                  });
                  return [subscriptionIDS, activeSubscriptionIDs];
                }
            };
        }()),

        /* Maintain advice action registry map as key(adviceName)and value(advice action callback function reference) pair*/
        ClientActionMap = (function() {
            var clientActions = new p.tools.Hashtable();
            return {
                add: function(actionName, action) {
                    clientActions.put(actionName, action);
                },
                get: function(actionName) {
                    return clientActions.get(actionName);
                },
                remove: function(actionName) {
                    clientActions.remove(actionName);
                },
                clear: function() {
                    clientActions.clear();
                }
        ...
        /* Processes the messages received from server and triggers message event behaviors on subscribtion element */
        MessageProcessor = (function() {
            var fireMessageEvent = function(subscriptionInfo, messageObj, websocketId) {
                    var data = messageObj.data;
                    var fromMessageProcessor = true;
                    if (subscriptionInfo && subscriptionInfo.subscriberElement) {
                        if (subscriptionInfo.callbackarguments) {
                            data.args = subscriptionInfo.callbackarguments;
                        }
                        if(subscriptionInfo.subscriberElement.getAttribute("data-ack-enabled")=="true"){
                            AcknowledgementManager.sendAcknowledgement(messageObj,messageObj.clientId, websocketId, "Callback-Intiated");
                        }
                        var targetPresent = subscriptionInfo.oWin && subscriptionInfo.oWin.pega.control.eventController.fireEventHandler(subscriptionInfo.subscriberElement, "message", data);
                        if (!targetPresent) {
                            SubscriptionManager.unregisterSubscriber(subscriptionInfo.subscriberElement.getAttribute("data-subscription-id"), false, fromMessageProcessor);
                        }
                    } else {
                        SubscriptionManager.unregisterSubscriber(messageObj.clientId, false, fromMessageProcessor);
                    }
                },
                processClientAction = function(actionName, data, clientId) {
                    if (actionName) {
                        var action = ClientActionMap.get(actionName);
                        if (action && typeof action === 'function') {
                            var callback = function(response, error) {
                                ...
                                var clientMessage = [];
                                clientMessage.push(clientResponse);
                                log("Client Response: " + JSON.stringify(clientMessage));
                                pega.u.MessagingManager.testObject.pushMessage(clientMessage);

                            }

                            var actionArgs = [data, callback];
                            try {
                                var actionResponse = action.apply(null, actionArgs);
                                if (actionResponse) {
                ...
                processAdvice = function(advice) {
                    var subscriptionData,
                        subscribtionsToRemove,
                        subcriptionIdsToVerify,
                        msg,
                        i;
                    if (advice.VerifySubscriptions) {

                        log("Advice message for handling duplicate subscribtions. " + advice.VerifySubscriptions);
                        subscribtionsToRemove = [];
                        subcriptionIdsToVerify = advice.VerifySubscriptions;

                        for (i = 0; i < subcriptionIdsToVerify.length; i++) {
                            subscriptionData = ActiveSubscriptionMap.get(subcriptionIdsToVerify[i]);
                            if (!subscriptionData || !subscriptionData.oWin || !subscriptionData.oWin.document.body.contains(subscriptionData.subscriberElement)) {
                                SubscriptionManager.unregisterSubscriber(subcriptionIdsToVerify[i], true);
                                subscribtionsToRemove.push(subcriptionIdsToVerify[i]);
                            }
                        }
                        if (subscribtionsToRemove.length > 0) {
                            msg = [{
                                "advice": {
                                    "CleanupSubscriptions": subscribtionsToRemove
                                }
                            }];
                            ConnectionManager.pushMessage(msg);
                        }
                    } else if(advice.notifyOperatorAdvice) {
                      log("Advice message for triggering debug pubsub activity");
                      // trigger debug activity
                      var paramsStr = "operatorId=" + advice.notifyOperatorAdvice + "&browserSnapshotData=" + JSON.stringify(ConnectionManager.getSubscriptionNSocketIDs());
                      pega.u.invoke.runActivity(["", paramsStr, "", "", "", "", "", pega.u.d.pushservice_encryptedActivityForWebsocket, ""]);
                   }
                },
                processMessage = function(messageObj, websocketId) {
                    if (messageObj.error) {
                        ErrorManager.processError(messageObj.error);
                        return;
                    }
                    if (messageObj.advice) {
                        processAdvice(messageObj.advice);
                        return;
                    }
                    if (messageObj.clientAction) {
                        processClientAction(messageObj.clientAction, messageObj.data, messageObj.clientId);
                        return
                    }
                    var activeSubscriptionNew = pega.u.MessagingManager.ActiveSubscriptionMap.get(messageObj.clientId) || window.top.pega.u.MessagingManager.ActiveSubscriptionMap.get(messageObj.clientId); //BUG-343329 : Taking the ActiveSubscription from the top window if not present in the current window
                    var areAcknowledgementsEnabledOnSubscriber = activeSubscriptionNew ? activeSubscriptionNew.subscriberElement.getAttribute("data-ack-enabled") : "false";
                    if(pega.ui.areAcknowledgementsEnabled || areAcknowledgementsEnabledOnSubscriber == "true"){
                      AcknowledgementManager.sendAcknowledgement(messageObj,messageObj.clientId, websocketId, "Message-Recieved");
                    }
                    fireMessageEvent(activeSubscriptionNew, messageObj, websocketId);
                };

            return {
                processMessageBundle: function(response, websocketId) {
                    try {
                        var messageArray = JSON.parse(response);
                        messageArray.forEach(function(message){processMessage(message, websocketId);});
                    } catch (e) {
                        log("Execption while processing response... " + e);
                    }
                },
                registerClientAction: function(actionName, actionFunRef) {
                    if (actionName) {
                        ClientActionMap.add(actionName, actionFunRef);
                    }
                },
                unregisterAllClientActions: function() {
                    ClientActionMap.clear();
                    log("Unregistered all client actions for the client");
                },
                unregisterClientAction: function(actionName) {
                    if (actionName) {
                        ClientActionMap.remove(actionName);
                        log("Unregistered client action for action name: " + actionName);
                    }
                }
            };
        }()),
        /* Manages the connection with server using atmosphere framework */
        ConnectionManager = (function() {
            var socket = p.atmosphere,
                subSocket,
                connected = false,
                bReconnectWebsockets = false,
                initialized = false,
                hasPortalname = (settings && settings.portalName && typeof settings.portalName == 'string' && settings.portalName.trim() != '') ? true : false,
                errorCode = '',
                request = {
                    url: document.location.protocol + "//" + document.location.host + settings.requestorContextPath + settings.pushServletPath + "?PZSRVRPSH=true&portalName=" + settings.portalName  + "&clientID=" + settings.requestorID,
                    contentType: "application/json",
                    transport: settings.transport,
                    fallbackTransport: settings.fallback,
                    maxReconnectOnClose: 5,
                    /* max no of retry for websockets */
                    reconnectInterval: 15000,
                    closeAsync: true,
                    trackMessageLength: true,
                    logLevel: 'warn',
                    maxRequest: 5,
                    /* max no of retry for long-polling */
                    reconnectingOnErrorProcess: false
                },
                websocketID = [];
            request.onOpen = function() {
                log("Websocket connected successfully!!! using transport: " + request.transport);
                if(websocketID.indexOf(this.uuid) === -1) {
                  websocketID.push(this.uuid);
                }
                connected = true;
                errorCode = '';
                bReconnectWebsockets = false;
                this.reconnectingOnErrorProcess = false;
                pega.u.clientEvents.publish(pega.u.clientEvents.WS_ON_OPEN, "Websocket connected successfully!!! using transport: " + request.transport);

            };
            request.onMessage = function(response) {
                log('Message :: ' + response.responseBody);
                var websocketId = response.request ? response.request.uuid : null;
                MessageProcessor.processMessageBundle(response.responseBody, websocketId);
            };
            ...
            request.onClose = function() {
                ...
                var websocketIndex = websocketID.indexOf(this.uuid);
                if(websocketIndex >= 0) {
                  websocketID.splice(websocketIndex, 1);
                }
                connected = false;
                if (errorCode && errorCode != '') {
                    ...
                    pega.u.clientEvents.publish(pega.u.clientEvents.WS_ON_CLOSE, msg);
                } else {
                    var msg = [{
                        "EventType": "onClose",
                        "Reason": "Websocket connection closed successfully!!!"
                    }];
                    pega.u.clientEvents.publish(pega.u.clientEvents.WS_ON_CLOSE, msg);
                }
            };
            //check for usage of response object 
            request.onReopen = function(response) {
                log("Websocket re-connection successful !!! using transport: " + response.transport);
                if(websocketID.indexOf(this.uuid) === -1) {
                  websocketID.push(this.uuid);
                }
                connected = true;
                ...
                bReconnectWebsockets = false;
                this.reconnectingOnErrorProcess = false;
                pega.u.clientEvents.publish(pega.u.clientEvents.WS_ON_REOPEN, "");
            };

            request.onError = function() {
                log("Some Error occurred in Atmosphere connection");
                if (bReconnectWebsockets && initialized && !connected) {
                    initialized = false;
                }
                pega.u.clientEvents.publish(pega.u.clientEvents.WS_ON_ERROR, "Some Error occurred in Websocket connection");
            }
            return {
                connect: function() {
                    if(!hasPortalname){
                      console.warn("Not subscribing to websockets as portal doesn't have a name");
                      return;
                    }
                    if (!initialized && !connected && (pega.u.d.ServerProxy && !pega.u.d.ServerProxy.isDestinationLocal())) {
                        try {
                            subSocket = socket.subscribe(request);
                            /* This flag is to initialize web socket connect only once */
                            initialized = true;
                        } catch (e) {
                            initialized = false;
                            log("Error occurred in Atmosphere connection");
                        }
                    }
                    bReconnectWebsockets = false;
                    request.reconnectingOnErrorProcess = false;
                    return connected;
                },
                disconnect: function() {
                ...
                setReconnect: function() {
                    bReconnectWebsockets = true;
                },
                /**Added below function as part of BUG-769655. This api will be used to reconnect the websocket when pega.u.clientEvents.WS_ON_CLOSE is registered. This api will be used by pega call team*/
                forceReconnect: function(){
                  initialized = true;
                  connected = false;
                  ConnectionManager.setReconnect();
                  ConnectionManager.tryReconnect();
                },
                /*Below Api is added as part of BUG-692543(INC-187617). This Api is used by pega call if websocket reconnect limit exceeds 5*/
                ...
                tryReconnect: function() {
                    if (bReconnectWebsockets == true) {
                        request.reconnectingOnErrorProcess = true;
                        ConnectionManager.connect();
                    }
                },
                setErrorCode: function(aErrorCode) {
                    errorCode = aErrorCode;
                },
                pushMessage: function(msgObj) {
                    if (msgObj && subSocket && connected) {
                        try {
                            var msg = JSON.stringify(msgObj);
                            log("Sending message to server : " + msg);
                            subSocket.push(msg);
                        } catch (ignore) {}
                    }
                },
                getSubscriptionNSocketIDs: function() {
                  var subscriptionIDs = ActiveSubscriptionMap.getAllActiveSubscriptionIDs();
                  return {
                    socketIDs: websocketID,
                    activeSubscriptionIDs: subscriptionIDs[1],
                    subscriptionIDs: subscriptionIDs[0]
                  }
                }
            };
        }()),
        /* Handles the errors using error codes recieved in the message from server */
        ErrorManager = (function() {
            var reconnectErrorCodeRegex = '^PE02';
            return {
                processError: function(errorObj) {
                    log("Error Message from server :: " + errorObj);
                    var errorCode = errorObj.split(":")[0];
                    /* We will disconnect the websockes for all the error codes send by the server. And will reconnect for the error codes starting with PE02 */
                    if (errorCode.match(reconnectErrorCodeRegex)) {
                        ConnectionManager.setReconnect();
                    }
                    ConnectionManager.setErrorCode(errorCode);
                    ConnectionManager.disconnect();
                }
            };
        }()),
         AcknowledgementManager = (function() {
          return {
            sendAcknowledgement: function(message, subscriptionId, websocketId, type) {
        ...
        /* Manages the registration and unregisteration of the subscription elements (uses ActiveSubscriptionMap) */
        SubscriptionManager = (function() {
            var WindowLocalSubscriptionsMap = {},
                isLogoutInProgress = function(){
                      var parentWindow = null;
                      if (pega && pega.mobile && pega.mobile.getDesktopWindow && typeof pega.mobile.getDesktopWindow === 'function') {
                        parentWindow = pega.mobile.getDesktopWindow();
                      } else if(pega && pega.ui && pega.ui.composer && pega.ui.composer.isActive()){
                        parentWindow = pega.ui.composer.getCurrentComposerWindow(); 
                      } else {
                        var tempWin = pega.desktop.support.getDesktopWindow ? pega.desktop.support.getDesktopWindow() : null;
                        parentWindow = tempWin ? tempWin : window;
                      }
                  return parentWindow.logout == true ? true : false;
                },
                sendCleanupMessage = function(subscriptionsToRemove){
                    var msg = [{
                                "advice": {
                                    "CleanupSubscriptions": subscriptionsToRemove
                                }
                            }];
                    ConnectionManager.pushMessage(msg);
                },
                sendUnregisterRequest = function(sid, deregisterUrl, acName) {
                    /* BUG-397551: START - send unsubscribe on parent thread for mdc subscriptions */
                    if (acName) {
                      acName += "_";
                      acName = acName.toUpperCase();
                    }
                    if(typeof(deregisterUrl) === "string" && deregisterUrl.indexOf(acName) !== -1) {
                       deregisterUrl = deregisterUrl.slice(0, deregisterUrl.indexOf("/" + acName));
                    }
                    /* BUG-397551: END */
                    var oSafeURL = SafeURL_createFromURL(deregisterUrl);
                    delete oSafeURL.hashtable.pzTransactionId;
                    oSafeURL.put("pyActivity", "pzUnsubscribeFromPush");
                    oSafeURL.put("subscriberId", sid);
                     if(!isLogoutInProgress()){
                      pega.util.Connect.asyncRequest('GET', oSafeURL.toURL());
                    }
                },
                windowUnloadAction = function(win) {
                    /* De register all subscriptions before the unload of window */
                    $(win).on('beforeunload', function() {
                         if(!isLogoutInProgress()){
                          p.u.MessagingManager.unregisterAll(win);
                        }
                        MessageProcessor.unregisterAllClientActions();
                    });
                };

            return {
                registerSubscriber: function(subscribtionID, subscriberElement, callbackarguments) {
                    if (subscribtionID && subscriberElement) {
                        var oWin = subscriberElement.ownerDocument.defaultView;
                        if (!oWin.msgMgrUniqueId) {
                            oWin.msgMgrUniqueId = "mmid" + (new Date()).getTime();
                            WindowLocalSubscriptionsMap[oWin.msgMgrUniqueId] = [];
                            windowUnloadAction(oWin);
                        }
                        WindowLocalSubscriptionsMap[oWin.msgMgrUniqueId].push(subscribtionID);
                        ActiveSubscriptionMap.add(subscribtionID, {
                            subscribtionID: subscribtionID,
                            subscriberElement: subscriberElement,
                            callbackarguments: callbackarguments,
                            oWin: oWin,
                            registrationUrl: oWin.pega.ctx.url,
                            threadName: oWin.pega.u.d.getThreadName(),
                            acName: oWin.pega.ctx.acName
                        });
                        log("Registered subscriber on client, id : " + subscribtionID);
                    }
                },
                unregisterSubscriber: function(subscribtionID, skipServerReq, fromMessageProcessor) {
					var returnValue;
                    if (subscribtionID) {
                        var sinfo = ActiveSubscriptionMap.remove(subscribtionID),
                            msgMgrUniqueId,
                            indx,
                            oWin = window;
                        if (sinfo) {
                            msgMgrUniqueId = sinfo.oWin.msgMgrUniqueId;
                            sinfo.subscriberElement = null;
                            sinfo.oWin = null;
                            if (msgMgrUniqueId) {
                                indx = WindowLocalSubscriptionsMap[msgMgrUniqueId].indexOf(subscribtionID);
                                if (indx !== -1) {
                                    WindowLocalSubscriptionsMap[msgMgrUniqueId].splice(indx, 1);
									returnValue = 1;
                                }
                            }
                            oWin = sinfo.oWin;
                        ...
                        if (!skipServerReq) {
                            var subArr = [];
                            if(fromMessageProcessor){
                              fromMessageProcessor = false;
                              //BUG-613116, if an element present in dom with id as data-subscription-id, don't add it in unsubscription list
                              if(document.querySelector("[data-subscription-id='"+subscribtionID+"']") == null){
                                subArr.push(subscribtionID);
                              }
                                
                            } else{
                              subArr.push(subscribtionID);
                            }
                            sendCleanupMessage(subArr);
                            /*
                            if (sinfo) {
                            	sendUnregisterRequest(subscribtionID, sinfo.registrationUrl, sinfo.acName);
                            } else {
                                sendUnregisterRequest(subscribtionID, oWin.pega.u.d.url, pega.ctx.acName);
                            }
                            */
                        }
                        log("Unregistered subscriber on client, id : " + subscribtionID);
                    }
					return returnValue;
                },
                unregisterAll: function(oWin,mdcThreadName) {
                    var slist,
                        Ids,
                        i;
                    if (oWin && oWin.msgMgrUniqueId) {
                        slist = WindowLocalSubscriptionsMap[oWin.msgMgrUniqueId];
                        if (slist) {
                            for (i = 0; i < slist.length; i++) {
                            	var purged = 0;
                            	if(mdcThreadName){
                            		var subsInfo = ActiveSubscriptionMap.get(slist[i]);
                            		if(subsInfo && subsInfo.threadName==mdcThreadName){
                            			purged = SubscriptionManager.unregisterSubscriber(slist[i], false);
                            		}
                            	}
                            	else{
                            		purged = SubscriptionManager.unregisterSubscriber(slist[i], false, mdcThreadName);
                            	}
                                
								// Check if loop array is modified, move pointer back
								if(purged == 1)
									i--;
                            }
                            if(!mdcThreadName){
	                            WindowLocalSubscriptionsMap[oWin.msgMgrUniqueId] = undefined
	                            oWin.msgMgrUniqueId = undefined;
	                        }
                        }
                        log("Unregistered All subscribers in the current window");
                    }
                },
                subscribe: function(classname, conditionProp, conditionVal, elementObject, onMessageCallback, callback) {
                    try {
                        if (elementObject == null || elementObject.document) {
                            var winObj = null;
                            if (elementObject == null) {
                                winObj = window;
                            } else {
                                winObj = elementObject;
                            }
                            var _elem = winObj.document.createElement("div");
                            _elem.setAttribute("style", "display:none;");
                            _elem.setAttribute("data-ctl", "");
                            winObj.document.body.appendChild(_elem);
                            elementObject = _elem;
                        }
                    } catch (e) {}
                    if (elementObject) {
                        var oWin = elementObject.ownerDocument.defaultView;
                        var oSafeURL = SafeURL_createFromURL(oWin.pega.ctx.url);
                        delete oSafeURL.hashtable.pzTransactionId;
                        oSafeURL.put("pyActivity", "pzSubscribeForNotification");
                        oSafeURL.put("channel", classname);
                        if (conditionProp == undefined || conditionProp == null || conditionProp == "") {
                            conditionProp = "null";
                        }

                        oSafeURL.put("conditionProp", conditionProp);
                        oSafeURL.put("conditionVal", JSON.stringify(conditionVal));
                        var postData = new SafeURL();
                        postData.put("pyReturnsResponse", "true");
                        var onSuccess = function(response) {
                            var subscriberId = response.responseText;
                            if (subscriberId && subscriberId != "" && elementObject.setAttribute) {
                                if (subscriberId.indexOf("<html") > -1) {
                                    log("Subscription Failed.....");
                                    return;
                                }
                                if (!elementObject.hasAttribute("data-ctl")) {
                                    elementObject.setAttribute("data-ctl", "");
                                }
                                elementObject.setAttribute("data-subscription-id", subscriberId);
                                elementObject.setAttribute("data-message", "[[\"runScript\", [\"" + onMessageCallback.funcRef + "(event.detail)\"]]]");
                                pega.ui.MessagingManager.registerSubscriber(subscriberId, elementObject, onMessageCallback.args);
                                if (callback && callback.success) {
                                    if (!callback.scope) {
                                        callback.success(subscriberId);
                                    } else {
                                        callback.success.apply(callback.scope, [subscriberId]);
                                    }
                                }
                            }
                        };
                        ...
                        oWin.pega.u.d.asyncRequest('POST', oSafeURL, _callback, postData);
                    }
                }
            };
        }());

    p.u.d.attachOnload(function() {
        /* Initiate connection to server on top PRPC window*/
        if (settings.transport) {
            ConnectionManager.connect();
        } else {
            log("Incorrect/No primary transport specified to initiate the atmosphere connection");
        }

    }, false);

    testObject.pushMessage = ConnectionManager.pushMessage;
    return {
        registerSubscriber: SubscriptionManager.registerSubscriber,
        unregisterSubscriber: SubscriptionManager.unregisterSubscriber,
        unregisterAll: SubscriptionManager.unregisterAll,
        subscribe: SubscriptionManager.subscribe,
        tryReconnect: ConnectionManager.tryReconnect,
        forceReconnect: ConnectionManager.forceReconnect,
        disconnect: ConnectionManager.disconnect,
        disconnectAndReconnect: ConnectionManager.disconnectAndReconnect,
        testObject: testObject,
        ActiveSubscriptionMap: ActiveSubscriptionMap,
        sendClientAcknowledgements: AcknowledgementManager.sendAcknowledgement,
        registerClientAction: MessageProcessor.registerClientAction,
        unregisterClientAction: MessageProcessor.unregisterClientAction,
        ClientActionMap: ClientActionMap
    };

}(pega, pega.u.pushservice_settings));
//static-content-hash-trigger-GCC