    ...
    if(pega.ctx.isMDC){
      Grids.deleteAllGridObjects();
		...
		*/
        ...
        /* HFIX-A431 Below changes is added to address a11y issue in report viewer (dynamic grid) */
...
if(!pega.ui.grid) {
      ...
      FLOW_ACTION : "FLOWACTION",
			...
			*/
			init: function() {
				...
				this.initGridConfig();
				this.initGridTables();
        this.dynamicGridHeaderA11y(); /* HFIX-A431 Call this method to fix header a11y issue in report viewer (dynamic grid) */
        ...
        /* If this is a virtual grid - make the first cell focusable */
        if(this.gridDiv.getAttribute("data-virtualrd") === 'true' && this.gridDiv.getAttribute("data-click") && this.gridDiv.getAttribute("oafunc") ) {
          try {
					if(rows[1] && !this.bTreegrid) {
						var first_row = Dom.hasClass(rows[1], "grid-categorize-header") ? 2 : 1;
            for(var index_row = first_row; index_row<rows.length; index_row++) {
	  					if(rows[index_row] && rows[index_row].cells[0] && !rows[index_row].cells[0].classList.contains('expandPane')) {             
                rows[index_row].cells[0].firstChild.role= "link";
                rows[index_row].cells[0].firstChild.tabIndex= "0";
                var txtcell = rows[index_row].cells[0].textContent;
                var oarags = rows[index_row].getAttribute("oaargs");
                if(oarags && oarags.indexOf(" ") !== -1) {
                  oarags = oarags.split(" ");
                  oarags = oarags[oarags.length - 1];
                  if(oarags !== txtcell) txtcell = txtcell + " " + oarags
                }
                rows[index_row].cells[0].firstChild.setAttribute("aria-label", txtcell);
                rows[index_row].cells[0].firstChild.setAttribute("data-ctl", "");
  						}
            }
          }
          } catch(e) {}
    ...
    if (!this.bTree && !this.bTreegrid && (this.fixedRow || this.pageMode == "Progressive Load") && Dom.hasClass(this.gridcontDiv, "gPercent") == true && this.rightHeaderTbl) {
        ...
        /* BUG-398688,BUG-405002: Do not markdirty for read-only details,pagination */
				if(action!="PAGINATE" && action!="SETFOCUS" && action!="OPEN" && !(action == 'EDITITEM' && (this.bRODetails || this.bReadOnlyShowDetails))){
					grid.markAsDirty();
				}
      ...
      hasFocusableContent: function(element) {
        ...
        pega.ui.statetracking.setBusy("gridEvent");
          ...
          if(eventKeyCode === 32) {
            /* HFIX-A431 Fix for report viewer(dynamic grid) filter that are menu button - if the key is spacebar - then simulate click to open the menu */
            if (target && target.id == "pui_colmenu") {
              ...
              return;
				...
				}else if(eventType == Grids.EVENT_CLICK){
...
//Added two conditions to if for validating initialization of variables
      ...
      /* HFIX-A431 fix header a11y issue in report viewer(dynamic grid) */
